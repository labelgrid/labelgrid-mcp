...
- `labelgrid analytics get --help` closes with a note that sections aggregate
  over the filter scope, so `--upc` returns release totals rather than per-track
  rows, and that per-track daily output comes from the `track-streams-daily` and
  `track-listeners-daily` sections.

### Fixed

- The command's header comment said the endpoint accepts section keys out of
  the 37 available. It takes 47.

## [0.3.1] - 2026-08-05

### Changed

- `labelgrid analytics get --platform` help text lists `AMAZON`, which the API
  accepts. The option has always been passed through to the API unchanged, so
  no previously working command changes behavior.
